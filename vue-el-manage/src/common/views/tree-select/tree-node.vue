  ...
  <div class="node-container" :class="{'close':!isOpen}" :style="{'min-width':`${minWidth-2}px`}">
    <div v-for="(item,index) in treeData" :key="index">
      <div  :style="{'padding-left':`${level*20+40}px`}" class="node" :class="{'can-not-click':item.canNotClick}" @click.stop="nodeClick(item,index)">
        <span v-if="item.children" class="icon" :style="{'left':`${level*20+15}px`,'background-image':`url('${needExpend[index]?unExpendIcon:expendIcon}')`}" @click.stop="expendClicked(item,index)"></span>
        {{item.name}}
      </div>
      <tree-node @defaultChosed="defaultChosed" @chosed="nodeClick" v-if="item.children" :treeData="item.children" :level="level+1" :isOpen="needExpend[index]" :defaultChosen="defaultChosen"></tree-node>
    </div>
  </div>
</template>
