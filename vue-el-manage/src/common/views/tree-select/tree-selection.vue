<template>
  <div class="tree-sel-container" :style="{'min-width':`${minWidth}px`}" v-clickoutside="handleClickOutSide">
    ...
    <span class="text" :chosedData="chosedData"  @click.stop="showOptions=!showOptions" :class="{'grey':!chosedData.id}">{{chosedData.name}}</span>
...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    mounted:function ()  {
      // bus.$on('depart-tree-input', function(id,name) {
      //   this.chosedData.name=name
      //   this.chosedData.id=id
      // }.bind(this));
    }
  }
</script>
