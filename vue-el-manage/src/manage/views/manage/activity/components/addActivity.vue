<template>
    <div class="currentDiv">
        ...
        <el-main class="content">
            <div>
                <el-form :rules="rules" :model="formInline" label-width="130px" ref="form">
...
<!--                    <el-button type="primary" @click="cancelInfo()">取 消</el-button>-->
                    <el-button type="primary" icon="el-icon-upload2" @click="showUpload()">自定义模板</el-button>
...
<!--        :before-close="handleClose"-->
        ...
        >
            <div style="width: 100%;text-align: center;margin-bottom: 25px;font-size: 16px;font-weight: 700;">
                自定义模板拓展
            </div>

            <el-form label-width="100px" ref="formWork" style="margin: 20px 0;" class="dialog-form">
                <div v-for="(item,index) in activityTemplate" :key="index">
                    <el-form-item :label="'选项'+parseFloat(index+1)">
                        ...
                        <el-button @click="removeItem(index)"
                                   :style="activityTemplate.length == 1 ? 'pointer-events: none;': ''"
...
export default {
    ...
    methods: {
        ...
        saveInfo() {
            this.$refs.form.validate(valid => {
                if (valid && this.dateVerify()) {
                    let form = {};
                    form.name = this.formInline.name;
                    form.title = this.formInline.title;
                    form.description = this.formInline.description;
                    form.artStartTime = this.formInline.artStartTime;
                    form.artEndTime = this.formInline.artEndTime;
                    form.activityTemplate = this.activityTemplate;
                    ...
                    this.$nextTick(async () => {
                        ...
                        if (res) {
                            this.$message({showClose: true, message: '活动创建成功！', type: 'success'});
                            ...
                            this.$message({showClose: true, message: '活动创建失败！', type: 'error'});
        ...
        removeItem(index) {
            this.activityTemplate.splice(index,1);
        },
        ...
        saveUpload() {
            this.activityTemplate.forEach((e, i) => {
                e.seq = i+1
                console.log("e.seq",e.seq)
            });
            this.dialog = false;
            console.log('this.activityTemplate==',this.activityTemplate);
