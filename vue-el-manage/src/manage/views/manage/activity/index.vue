...
<!--        <activity-manage></activity-manage>-->
        <div :class="control.state.showInfoStyle" v-show="control.state.isShow">
            <activity-manage ref="activityManage" :on-add="addBtn"
                             :on-detail="detailBtn"
                             :on-edit="editBtn"
            ></activity-manage>
        </div>
        ...
        <div class="panel-group" :class="control.state.detailInfoStyle" v-show="control.state.isDetail">
            <el-row class="  animated pulse " :gutter="20">
                <activity-detail  ref="activityDetailManage" :onBack="AddCancelBtn"></activity-detail>
            </el-row>
        </div>

        <div class="panel-group" :class="control.state.addInfoStyle" v-show="control.state.isAdd">
            <el-row class="  animated pulse " :gutter="20">
                <add-activity ref="addActivity" :onBack="AddCancelBtn"></add-activity>
            </el-row>
        </div>
...
<!--                <edit-memeber ref="editMember" :onBack="AddCancelBtn"></edit-memeber>-->
                <edit-activity ref="editActivity" :onBack="AddCancelBtn"></edit-activity>
            </el-row>
        </div>
...
export default {
    ...
    methods: {
        detailBtn: function (row) {
            ...
            this.$refs.activityDetailManage.init(row);
        },
        AddCancelBtn: function (val) {
            if (val == 'succ')
                this.$refs.activityManage.getFirstTableData();
        ...
        editBtn: function (row) {
            ...
            this.$refs.editActivity.init(row);
        }
    }
