<template>
    <div class="container">
        <div :class="control.state.showInfoStyle" v-show="control.state.isShow">
            <inform-manage ref="informManage" :onAdd="addBtn" :onEdit="editBtn" :onDetail="detailBtn"></inform-manage>
        </div>
...
import InformManage from "./components/InformManage";
...
export default {
    ...
    components: {
        InformManage, AddInform, EditInform, InformDetail
    },
    watch: {
    ...
    methods: {
        ...
        editBtn: function (id) {
            ...
            this.$refs.editInform.load(id);
        },

        detailBtn: function (id) {
            this.$store.commit('SET_FORWARD_STYLE', 'detail');
            this.$refs.informDetailsManage.load(id);
        },

        backBtn: function (val) {
            if (val == "succ")
                this.$refs.informManage.getFirstTableData();
