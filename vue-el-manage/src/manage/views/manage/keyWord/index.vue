<template>
    <div class="container">
        <div :class="control.state.showInfoStyle" v-show="control.state.isShow">
            <KeyWordDialog ref="keyWordDialog" :on-add="addBtn" :on-edit="editBtn"></KeyWordDialog>
        </div>

        <div :class="control.state.addInfoStyle" v-show="control.state.isAdd">
            <add-key-word ref="addKeyWord" :on-back="AddCancelBtn"></add-key-word>
        </div>

        <div :class="control.state.editInfoStyle" v-show="control.state.isEdit">
            <edit-key-word ref="editKeyWord" :on-back="AddCancelBtn"></edit-key-word>
        </div>
    </div>
</template>
<script>
...
export default {
    ...
    components: {
        KeyWordDialog, AddKeyWord, EditKeyWord
    },
    watch: {},
    created() {
    ...
    methods: {
        ...
        editBtn: function (id) {
            ...
            this.$refs.editKeyWord.getInfoById(id);
        },

        AddCancelBtn: function (val) {
            if (val == 'succ') {
                this.$refs.keyWordDialog.getSelect();
                this.$refs.keyWordDialog.getDictionary();
            }
