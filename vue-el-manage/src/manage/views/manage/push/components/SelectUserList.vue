<template>
    <div>
        <div v-if="showPage">
            ...
            <show-user-list ref="showUserList"></show-user-list>   <!-- 查看通知用户 弹框 -->
            ...
            <div>
                <div class="userDataDiv">
                    ...
                    >
                        ...
                        <el-table-column label="用户编号" prop="realName" width="200"></el-table-column>
...
export default {
    ...
    data() {
        return {
            ...
            selectedEnsureRealName: [], //确定选中的用户编号数据
    ...
    methods: {
        changeSelected(val) {
            // let _that = this;
            // let arr = new Array();
            // console.log('oooooo',this.selectedEnsureRealName);
            if (this.selectedUserList.length > 0 && this.originalSelectedUserList.length > 0) {
                // if (this.selectedEnsureRealName.length > 0 && this.originalSelectedUserList.length > 0) {
                // this.originalSelectedUserList.forEach(function (e, i) {
                //     if (_that.selectedUserList.indexOf(e.realName) != -1) {
                //         arr.push(e);
                //     }
                // });
        ...
        toggleSelection(rows) {
            if (rows) {
                ...
                rows.forEach(row => {
                    console.log('rowrowrowrow',row,this.firstTableData)
        ...
        showOpenObjs(operRow, functionPoint) {
            if (this.selectedEnsureRealName.length == 0) {
                this.$message({
                    message: '选择的用户为空',
                    type: 'warning'
                });
            ...
            this.$refs.showUserList.load(this.selectedEnsureRealName);
        },
        ...
        handleSelect(val,row){
            ...
            // 判断数组中是否有某个元素存在
            // console.log('row:',row)
            let isValue = this.selectedUserList.some(item =>{
                if(item.openId == row.openId){
                    return true;
                }
            })
            console.log('isValue:',isValue)
            if(isValue){ //如果selectdUserList本身就有这个元素，那么现在就要删除
                let index =0;
                for(let i = 0;i<this.selectedUserList.length;i++){
                    if(this.selectedUserList[i].openId.indexOf(row.openId) > -1){
                        index = i;
            ...
            //     this.selectedUserList.push(arr[m])
            // }


            // console.log('触发点击事件：',this.selectedUserList)
        },
        //当用户手动勾选全选 Checkbox 时触发的事件
        selectAll(val){
            // console.log('当用勾选全选按钮时：',val)
           // 如果选中 全选按钮
           if(val.length > 0){
               for (let i = 0; i < val.length; i++) {
                   let tag = {
           ...
           //    删除 当页 的所有选中数据
               for(let i = 0;i<this.selectedUserList.length;i++){
                   for(let j = 0;j<this.firstTableData.length;j++){ //当页的所有数据
                       if(this.selectedUserList[i].openId == this.firstTableData[j].openId){
         ...
         */
        submitSelected() {
            let RealName = new Array();
            if (this.selectedUserList.length > 0) {
                this.selectedUserList.forEach(function (e, i) {
                    RealName.push(e.realName);
                });
                this.selectedEnsureRealName = RealName;
            } else {
                this.selectedEnsureRealName = [];
            }
            this.isSearch = false;
            console.log('selectedUserList',this.selectedUserList,this.selectedEnsureRealName)
        },
        ...
        getFirstTableData() {
            ...
            this.$nextTick(async () => {
                ...
                console.log('this.selectedUserList请求调用：',this.selectedUserList)

                let firstTableDataSelectedRow = []; // 表格数据 选中行，判断该行是否选中
                for(let i = 0;i<this.firstTableData.length;i++){
                    for(let j = 0;j<this.selectedUserList.length;j++){
                        if(this.selectedUserList[j].openId == this.firstTableData[i].openId){
                            firstTableDataSelectedRow.push(this.firstTableData[i])  //保存选中行记录
                        }
                    }
                }

                this.changeSelected(firstTableDataSelectedRow);
            });
        },
