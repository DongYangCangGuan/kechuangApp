                    ...
                    <el-row class="panel-group" :gutter="24">
                        <el-col>
                            <el-form-item label="发布类型：" prop="taskType">
                                <el-radio-group v-model="formInline.taskType" @change="taskTypeRadioChanges()" size="small">
...
export default {
    ...
    data() {
        return {
            ...
            formInline: {
                ...
                remark:'',
                taskType: '', //单选按钮-类型消息
                sendType:'6', //发布类型-类型消息 默认6 表示没有选中
                openIds:[],   //发布类型-用户消息 放着选中用户的openId
            },
            files: [],
            options2:[
                {
    ...
    methods: {
        ...
        taskTypeRadioChanges() {
            ...
            // }
            if (this.formInline.taskType == 'user') {
                this.$refs.selectUserList.showPage = true;
                this.$refs.selectTypeList.showPage = false;
            }else if(this.formInline.taskType == 'type'){
        ...
        backBtn: function () {
            if (this.onBack) {
                ...
                this.$refs.selectTypeList.showPage = false;  //恢复发布类型单选的默认状态
                this.$refs.selectUserList.showPage = false;
                this.$refs.selectUserList.selectedUserList = []; //清除上次已选中的用户数据
        ...
        saveInfo() {
            this.$refs.form.validate(valid => {
                if (valid) {
                    // console.log('父访问子selectUserList111:',this.$refs.selectUserList.selectedUserList)
                    //发布类型默认传给后台的值
                    this.formInline['openIds'] = [];
                    this.formInline['sendType'] = 6
                    if(this.$refs.selectUserList.showPage){
                        let openIds = []
                        this.$refs.selectUserList.selectedUserList.map((item) => {
                            openIds.push(item.openId)
                        })
                        this.formInline['openIds'] = openIds
                    }
                    if(this.$refs.selectTypeList.showPage){
                        this.formInline['sendType']= this.$refs.selectTypeList.value
                    }
